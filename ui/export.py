...
def export_pdf(result: dict, filename: str = "") -> bytes:
    ...
    branding_path = Path("assets/branding_image.png")
    if not branding_path.exists():
        branding_path = Path(__file__).resolve().parent.parent / "assets" / "branding_image.png"

    story.append(Spacer(1, 40))
    if logo_path.exists():
        try:
            story.append(Image(str(logo_path), width=150, height=45, hAlign='CENTER'))
    ...
    if branding_path.exists():
        try:
            story.append(Spacer(1, 30))
            story.append(Image(str(branding_path), width=320, height=180, hAlign='CENTER'))
        except Exception:
            pass
